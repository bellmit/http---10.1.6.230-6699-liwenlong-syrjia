...
TouchScroll.prototype={
    ...
    /*??????????????????§¹??
     *@param Number offx: ???????????offx????
     *@param Number offy: ????????????offy????
     */
    ...
    start:function(e){
        clearTimeout(this.timer[0]); clearTimeout(this.timer[1]);//?????????§Ö????
