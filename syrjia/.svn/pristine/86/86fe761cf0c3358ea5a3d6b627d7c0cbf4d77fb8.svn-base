...
app.controller('circleCon', [ '$scope', '$location', '$http', '$sce', '$filter', function($scope, $location, $http, $sce, $filter) {
	...
	http = $http;
	var qrCode;
	$http.post(basePath + 'knowledgeCircle/queryCircleDetail.action', {
		id : circle
	}, postCfg).then(
		function(response) {
			if (response.data && response.data.respCode == 1001) {
				...
				$http.post(basePath + 'im/queryScan.action', {
					url : $location.absUrl()
				}, postCfg).then(
					function(response) {
						if (response.data && response.data.respCode == 1001) {
							...
							wx.ready(function() {
								// 微信分享的数据  
								var shareData = {
									"imgUrl" :  null != $scope.data.imgUrl&&$scope.data.imgUrl!='' ?$scope.data.imgUrl:null != $scope.data.videoShowUrl&&$scope.data.videoShowUrl!=''?$scope.data.videoShowUrl:null == $scope.data.docUrl ? 'https://mobile.syrjia.com/syrjia/img/logo.png' : $scope.data.docUrl, // 分享显示的缩略图地址  
									"link" : $location.absUrl(), // 分享地址  
									"desc" : $scope.data.content.replace(/<\/?.+?>/g, "").replace(/ /g, "").replace(/&nbsp;/ig, ''), // 分享描述  
									"title" : $scope.data.title, // 分享标题  
									success : function() {
										//alert("分享成功"); 
										$http.post(basePath + 'knowledgeCircle/addUserShare.action', {
											goodsId : $location.search().circle
										}, postCfg)
		...
		pullRefresh : {
			...
			up : {
				...
				callback : function() { // 必选，刷新函数，根据具体业务来编写，比如通过ajax从服务器获取新数据；
					var _this = this;
					page++;
					$http.post(basePath + 'knowledgeCircle/queryEvaList.action', {
						id : circle,
						page : page,
						row : row
					}, postCfg)
	...
	$scope.Foll = function(event, id, num) {
		...
		$http.post(basePath + 'knowledgeCircle/updateFollow.action', {
			id : id,
			num : num
		}, postCfg).then(
			function(response) {
		...
		$.ajax({
		    ...
		    success: function (data) {
		    	if(data.respCode!=1001){
					var content='<div class="layer subscribeLayer"></div><div class="public_twocode subscribeQrcode"><a></a><p class="public_top">关注上医仁家平台</p><p class="public_title public_bot public_ichrscan">长按识别二维码</p><div class="public_code"><span> <img src="'+qrCode+'"> </span></div><p class="public_title">为了您更好的享受上医仁家的服务，如实时的接受调理、购物等通知，请您关注上医仁家平台。</p></div>';
					$("body").append(content);
				}else {
					if(data.data.state==2){
						...
						if (tab == 1) {
							window.location.href = "../hospital/doctor_detail.html?id=" + id + "&num" + "=2";
						} else {
	...
	$scope.comment = function() {
		...
		if (type == 1) {
			...
			$http.post(basePath + 'knowledgeCircle/addEvaReply.action', {
				knowId : $scope.knowId,
				content : content,
				type : 1,
				pid : 0,
				...
				function(response) {
					...
					if (response.data && response.data.respCode == 1001) {
						if (response.data.data.result == 1) {
							openAlert("评论成功!");
							$http.post(basePath + 'knowledgeCircle/queryEva.action', {
								id : circle
							}, postCfg).then(
								function(response) {
									if (response.data && response.data.respCode == 1001) {
