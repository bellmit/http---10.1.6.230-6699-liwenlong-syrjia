package cn.syrjia.config;

public class configCode {

	/**
	 * 成功
	 */
	public static final int code_1001 = 1001;

	public static final int code_1002 = 1002;

	public static final int code_1003 = 1003;

	public static final int code_1004 = 1004;

	public static final int code_1005 = 1005;

	public static final int code_1006 = 1006;

	public static final int code_1007 = 1007;

	public static final int code_1008 = 1008;

	public static final int code_1009 = 1009;

	public static final int code_1010 = 1010;

	public static final int code_1011 = 1011;

	public static final int code_1012 = 1012;

	public static final int code_1013 = 1013;

	public static final int code_1014 = 1014;

	public static final int code_1015 = 1015;

	public static final int code_1016 = 1016;

	public static final int code_1017 = 1017;
	public static final int code_1018 = 1018;
	public static final int code_1019 = 1019;
	public static final int code_1020 = 1020;
	public static final int code_1021 = 1021;
	public static final int code_1022 = 1022;
	public static final int code_1023 = 1023;
	/**
	 * 订单已支付
	 */
	public static final int code_1024 = 1024;
	/**
	 * 订单已完成
	 */
	public static final int code_1025 = 1025;

	/**
	 * 订单已取消
	 */
	public static final int code_1026 = 1026;

	/**
	 * 订单已退款
	 */
	public static final int code_1027 = 1027;

	/**
	 * 订单未支付
	 */
	public static final int code_1028 = 1028;

	/**
	 * 参数不正确
	 */
	public static final int code_1029 = 1029;

	/**
	 * 充值失败
	 */
	public static final int code_1030 = 1030;
	/**
	 * 付款失败
	 */
	public static final int code_1031 = 1031;

	/**
	 * 医生信息不存在
	 */
	public static final int code_1032 = 1032;

	/**
	 * 加入订单失败
	 */
	public static final int code_1033 = 1033;

	public static final int code_1034 = 1034;

	public static final int code_1035 = 1035;

	public static final int code_1036 = 1036;

	public static final int code_1037 = 1037;

	public static final int code_1038 = 1038;

	public static final int code_1039 = 1039;

	public static final int code_1040 = 1040;

	public static final int code_1041 = 1041;

	public static final int code_1042 = 1042;

	public static final int code_1043 = 1043;
	/**
	 * 验证码发送失败
	 * 
	 */
	public static final int code_1044 = 1044;
	/**
	 * 验证码不正确
	 * 
	 */
	public static final int code_1045 = 1045;
	/**
	 * 验证码超时
	 * 
	 */
	public static final int code_1046 = 1046;
	public static final int code_1047 = 1047;
	public static final int code_1048 = 1048;

	public static final int code_1049 = 1049;

	public static final int code_1050 = 1050;
	/**
	 * 注销登陆失败
	 */
	public static final int code_1051 = 1051;

	public static final int code_1052 = 1052;

	public static final int code_1053 = 1053;
	/**
	 * 很抱歉，医生已关闭线上图文咨询服务
	 */
	public static final int code_1054 = 1054;
	/**
	 * 很抱歉，医生已关闭线上电话调理服务
	 */
	public static final int code_1055 = 1055;
	/**
	 * 很抱歉，医生已关闭线上电话咨询服务
	 */
	public static final int code_1056 = 1056;
	/**
	 * 很抱歉，医生已关闭线上调理服务
	 */
	public static final int code_1057 = 1057;
	/**
	 * 订单生成失败
	 */
	public static final int code_1058 = 1058;
	public static final int code_1059 = 1059;
	public static final int code_1060 = 1060;
	public static final int code_1061 = 1061;
	public static final int code_1062 = 1062;
	public static final int code_1063 = 1063;
	public static final int code_1064 = 1064;
	public static final int code_1065 = 1065;
	public static final int code_1066 = 1066;
	/**
	 * 商品不存在
	 */
	public static final int code_1067 = 1067;

	public static final int code_1068 = 1068;

	public static final int code_1069 = 1069;

	public static final int code_1070 = 1070;

	public static final int code_1071 = 1071;

	public static final int code_1072 = 1072;

	public static final int code_1073 = 1073;

	public static final int code_1074 = 1074;

	public static final int code_1075 = 1075;

	public static final int code_1076 = 1076;

	public static final int code_1077 = 1077;

	public static final int code_1078 = 1078;

	public static final int code_1079 = 1079;

	public static final int code_1080 = 1080;

	public static final int code_1081 = 1081;

	public static final int code_1082 = 1082;

	public static final int code_1083 = 1083;

	public static final int code_1084 = 1084;

	public static final int code_1085 = 1085;

	public static final int code_1086 = 1086;

	public static final int code_1087 = 1087;

	public static final int code_1088 = 1088;

	public static final int code_1089 = 1089;

	public static final int code_1090 = 1090;

	public static final int code_1091 = 1091;

	/**
	 * 助理信息不存在或被禁用
	 */
	public static final int code_1092 = 1092;

	/**
	 * 新密码输入不正确
	 */
	public static final int code_1093 = 1093;

	public static final int code_1094 = 1094;

	public static final int code_1095 = 1095;

	public static final int code_1096 = 1096;

	public static final int code_1097 = 1097;

	public static final int code_1098 = 1098;

	public static final int code_1099 = 1099;

	public static final int code_1100 = 1100;

	public static final int code_1101 = 1101;

	public static final int code_1102 = 1102;

	public static final int code_1103 = 1103;

	public static final int code_1104 = 1104;

	public static final int code_1105 = 1105;

	public static final int code_1106 = 1106;

	public static final int code_1107 = 1107;

	public static final int code_1108 = 1108;

	public static final int code_1109 = 1109;

	public static final int code_1110 = 1110;

	public static final int code_1111 = 1111;

	public static final int code_1112 = 1112;

	public static final int code_1113 = 1113;

	public static final int code_1114 = 1114;

	public static final int code_1115 = 1115;

	public static final int code_1116 = 1116;

	public static final int code_1117 = 1117;

	public static final int code_1118 = 1118;

	public static final int code_1119 = 1119;

	public static final int code_1120 = 1120;
	
	public static final int code_1121 = 1121;
	
	public static final int code_1122 = 1122;
	
	public static final int code_1123 = 1123;
	
	public static final int code_1124 = 1124;
	
	public static final int code_1125 = 1125;
	
	public static final int code_1126 = 1126;
	
	public static final int code_1127 = 1127;
	
	public static final int code_1128 = 1128;
	
	public static final int code_1129 = 1129;
	
	public static final int code_1130 = 1130;
	
	public static final int code_1131 = 1131;
	
	public static final int code_1132 = 1132;
	
	public static final int code_1133 = 1133;
	
	public static final int code_1134 = 1134;
	
	public static final int code_1135 = 1135;
	
	public static final int code_1136 = 1136;
	
	public static final int code_1137 = 1137;
	
	public static final int code_1140 = 1140;
	
	public static final int code_1141 = 1141;
	
	public static final int code_1142 = 1142;
	
	public static final int code_1143 = 1143;
	
	public static final int code_1144 = 1144;

	public static String codeDesc(int code) {
		String codeDesc = "成功！";
		switch (code) {
		case 1001:
			codeDesc = "成功！";
			break;
		case 1002:
			codeDesc = "用户不存在！";
			break;
		case 1003:
			codeDesc = "手势密码不正确！";
			break;
		case 1004:
			codeDesc = "用户已存在！";
			break;
		case 1005:
			codeDesc = "数字密码不正确！";
			break;
		case 1006:
			codeDesc = "修改密码失败";
			break;
		case 1007:
			codeDesc = "注册成功";
			break;
		case 1008:
			codeDesc = "用户名或密码不正确";
			break;
		case 1009:
			codeDesc = "用户已被禁用";
			break;
		case 1010:
			codeDesc = "新增失败";
			break;
		case 1011:
			codeDesc = "获取信息失败";
			break;
		case 1012:
			codeDesc = "该手机号未注册";
			break;
		case 1013:
			codeDesc = "认证提交失败";
			break;
		case 1014:
			codeDesc = "编辑失败";
			break;
		case 1015:
			codeDesc = "服务器异常，请重试";
			break;
		case 1016:
			codeDesc = "未查到相应医生职称信息";
			break;
		case 1017:
			codeDesc = "未查到相应患者信息";
			break;
		case 1018:
			codeDesc = "未获取到登录信息，请先登录";
			break;
		case 1019:
			codeDesc = "每月仅可群发一次";
			break;
		case 1020:
			codeDesc = "一天内最大发送上限为5条";
			break;
		case 1021:
			codeDesc = "商品数量不足，请重新选择";
			break;
		case 1022:
			codeDesc = "原密码不正确，请重新输入";
			break;
		case 1023:
			codeDesc = "订单已取消或未付款";
			break;
		case 1024:
			codeDesc = "订单已支付";
			break;
		case 1025:
			codeDesc = "订单已完成";
			break;
		case 1026:
			codeDesc = "订单已取消";
			break;
		case 1027:
			codeDesc = "订单已退款或退款中";
			break;
		case 1028:
			codeDesc = "订单未支付";
			break;
		case 1029:
			codeDesc = "参数错误";
			break;
		case 1030:
			codeDesc = "该用户已注册过";
			break;
		case 1031:
			codeDesc = "新手机号与原手机号不能相同";
			break;
		case 1032:
			codeDesc = "医生信息不存在或不在服务时间";
			break;
		case 1033:
			codeDesc = "加入订单失败";
			break;
		case 1034:
			codeDesc = "订单信息不存在";
			break;
		case 1035:
			codeDesc = "取消订单失败";
			break;
		case 1036:
			codeDesc = "退款申请提交失败";
			break;
		case 1037:
			codeDesc = "订单状态变更失败";
			break;
		case 1038:
			codeDesc = "原手机号不能为空";
			break;
		case 1039:
			codeDesc = "订单付款状态变更失败";
			break;
		case 1040:
			codeDesc = "新手机号不能为空";
			break;
		case 1041:
			codeDesc = "医生已被禁用";
			break;
		case 1042:
			codeDesc = "该手机号已存在";
			break;
		case 1043:
			codeDesc = "输入手机号与原手机号不匹配";
			break;
		case 1044:
			codeDesc = "验证码发送失败";
			break;
		case 1045:
			codeDesc = "验证码错误，请重新发送";
			break;

		case 1046:
			codeDesc = "验证码已过期";
			break;
		case 1047:
			codeDesc = "手机号或密码为空";
			break;
		case 1048:
			codeDesc = "上传失败";
			break;
		case 1049:
			codeDesc = "订单待支付状态超时，系统已取消订单";
			break;
		case 1050:
			codeDesc = "暂无患者信息";
			break;
		case 1051:
			codeDesc = "注销登陆失败";
			break;
		case 1052:
			codeDesc = "该微信用户已存在！";
			break;
		case 1053:
			codeDesc = "账号不匹配！";
			break;
		case 1054:
			codeDesc = "很抱歉，医生已关闭线上图文咨询服务";
			break;
		case 1055:
			codeDesc = "很抱歉，医生已关闭线上电话调理服务";
			break;
		case 1056:
			codeDesc = "很抱歉，医生已关闭线上电话咨询服务";
			break;
		case 1057:
			codeDesc = "很抱歉，医生已关闭线上调理服务";
			break;
		case 1058:
			codeDesc = "订单生成失败";
			break;
		case 1059:
			codeDesc = "医生已被锁定或已离职";
			break;
		case 1060:
			codeDesc = "订单价格为0，不可支付";
			break;
		case 1061:
			codeDesc = "验签失败";
			break;
		case 1062:
			codeDesc = "验签失败,时间超时";
			break;
		case 1063:
			codeDesc = "请在微信浏览器中打开";
			break;
		case 1064:
			codeDesc = "登录失败";
			break;
		case 1065:
			codeDesc = "已有拍方抓药申请未处理，不可重复申请";
			break;
		case 1066:
			codeDesc = "操作失败";
			break;
		case 1067:
			codeDesc = "商品不存在";
			break;
		case 1068:
			codeDesc = "加入购物车失败";
			break;
		case 1069:
			codeDesc = "收货地址不存在";
			break;
		case 1070:
			codeDesc = "验证码3分钟内有效";
			break;
		case 1071:
			codeDesc = "订单不存在";
			break;
		case 1072:
			codeDesc = "您正处于服务中";
			break;
		case 1073:
			codeDesc = "本商品不支持购物车购买";
			break;
		case 1074:
			codeDesc = "您还未登录，请先登陆后再操作";
			break;
		case 1075:
			codeDesc = "获取sig失败";
			break;
		case 1076:
			codeDesc = "验证码获取次数超出限制，每天最多获取5次";
			break;
		case 1077:
			codeDesc = "此账号已在其他客户端登陆";
			break;
		case 1078:
			codeDesc = "token失效";
			break;
		case 1079:
			codeDesc = "超出限购数量";
			break;
		case 1080:
			codeDesc = "商品库存不足";
			break;
		case 1081:
			codeDesc = "订单已评价";
			break;
		case 1082:
			codeDesc = "商品价格不正确,请重新下单";
			break;
		case 1083:
			codeDesc = "订单已支付或已取消";
			break;
		case 1084:
			codeDesc = "订单支付失败";
			break;
		case 1085:
			codeDesc = "添加商品失败，已超出购物车最大容量";
			break;
		case 1086:
			codeDesc = "该订单已填写症状描述或已过期";
			break;
		case 1087:
			codeDesc = "评价失败";
			break;
		case 1088:
			codeDesc = "失败";
			break;
		case 1089:
			codeDesc = "该药品已下架或不存在";
			break;
		case 1090:
			codeDesc = "无法回复自己的评论!";
			break;
		case 1091:
			codeDesc = "常用语最多添加50条";
			break;
		case 1092:
			codeDesc = "助理信息不存在或已被禁用";
			break;
		case 1093:
			codeDesc = "新密码输入不正确";
			break;
		case 1094:
			codeDesc = "问诊单或复诊单信息不存在";
			break;
		case 1095:
			codeDesc = "消息不存在";
			break;
		case 1096:
			codeDesc = "调理方案不存在或已删除";
			break;
		case 1097:
			codeDesc = "获取支付信息失败";
			break;
		case 1098:
			codeDesc = "您尚未设置医馆服务，请先设置服务内容";
			break;
		case 1099:
			codeDesc = "很抱歉，医生已关闭接单服务";
			break;
		case 1100:
			codeDesc = "请先进行医师认证";
			break;
		case 1101:
			codeDesc = "暂无物流信息";
			break;
		case 1102:
			codeDesc = "解析失败";
			break;
		case 1103:
			codeDesc = "未查询到您的助理信息";
			break;
		case 1104:
			codeDesc = "证书等图像信息不能为空";
			break;
		case 1105:
			codeDesc = "调理方正在修改，无法支付";
			break;
		case 1106:
			codeDesc = "签名参数错误";
			break;
		case 1107:
			codeDesc = "无法回复自己的评论!";
			break;
		case 1108:
			codeDesc = "用户注册失败";
			break;
		case 1109:
			codeDesc = "调理方已付款，不能作废";
			break;
		case 1110:
			codeDesc = "调理方已付款，不能修改";
			break;
		case 1111:
			codeDesc = "调理方已作废";
			break;
		case 1112:
			codeDesc = "调理方已付款，不能修改";
			break;
		case 1113:
			codeDesc = "医生姓名不能为空";
			break;
		case 1114:
			codeDesc = "医生已有待审核简介";
			break;
		case 1115:
			codeDesc = "收货地址最多创建100个";
			break;
		case 1116:
			codeDesc = "医生该类服务每日接单数已达上限";
			break;
		case 1117:
			codeDesc = "新密码不可与原密码相同";
			break;
		case 1118:
			codeDesc = "超出每天最大拨打限制次数";
			break;
		case 1119:
			codeDesc = "医院信息不存在";
			break;
		case 1120:
			codeDesc = "同一坐诊时间不能添加重复的坐诊地点 ";
			break;
		case 1121:
			codeDesc = "密码未设置，请先设置";
			break;
		case 1122:
			codeDesc = "认证资料已待审核";
			break;
		case 1123:
			codeDesc = "短信发送失败";
			break;
		case 1124:
			codeDesc = "请先设置费用";
			break;
		case 1125:
			codeDesc = "你已被禁言,请确认该账号是否正常";
			break;
		case 1126:
			codeDesc = "最多可添加5个就诊人";
			break;
		case 1127:
			codeDesc = "已有现场抄方申请未处理，不可重复申请";
			break;
		case 1128:
			codeDesc = "患者已支付调理方，无法修改";
			break;
		case 1129:
			codeDesc = "医生信息不存在或已被禁用";
			break;
		case 1130:
			codeDesc = "此医生该服务已达日接单上限";
			break;
		case 1131:
			codeDesc = "此手机在平台不唯一，如需登录，请联系客服";
			break;
		case 1132:
			codeDesc = "补充调理费应为大于0";
			break;
		case 1133:
			codeDesc = "订单已申请退款，不可进行支付";
			break;
		case 1134:
			codeDesc = "订单已部分退款，不可进行支付";
			break;
		case 1135:
			codeDesc = "退款金额不可大于实际支付金额";
			break;
		case 1136:
			codeDesc = "患者已确认订单，不可进行修改";
			break;
		case 1137:
			codeDesc = "已有补缴挂号费订单，不可重复发送";
			break;
		case 1140:
			codeDesc = "删除关联订单失败";
			break;
		case 1141:
			codeDesc = "获取药品信息失败";
			break;
		case 1142:
			codeDesc = "很抱歉，未查到您的相关医生信息";
			break;
		case 1143:
			codeDesc = "很抱歉，未查到您的所属医珍堂信息，请联系客服";
			break;
		case 1144:
			codeDesc = "系统检测到您的APP版本不是最新版，请前往下载最新版本";
			break;	
		}
		return codeDesc;
	}
}
